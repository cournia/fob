...
///////////////////////////////////////////////////////////////////////////////
void 
fob::bird::set_rotation( const glm::quat& quat )
{
    m_rotation = quat;
}
...
fob::bird::update_orientation( void )
{
    ...
    //not the global one
    glm::vec3 axis;
	tmp = glm::quat(0.0, glm::vec3(0, 0, 1) );
	glm::quat qtov = m_quaternion * tmp * glm::inverse(m_quaternion);
...
fob::read( unsigned char *buffer, int bytes )
{
    ...
    for( i = 0; (i < bytes) && (read_error < 5); ) {
        //read in 1 byte at a time
        //std::cout<< "read: " << i << " " << bytes << " " << read_error << std::endl;
        //count = ::read( m_device, &buffer[ i ], 1 );
		count = RS232_PollComport(m_device, &buffer[i], 1);
        if( count < 0 ) {
            //error
            if( errno != EINTR ) {
...
int fob::write(int device, unsigned char* buffer, int numofbytes)
{
	int n=0;
	n = RS232_SendBuf(device, buffer,numofbytes);
	return n;
}
...
fob::check_address_mode( void )
{
    ...
    std::cout <<  "fob::check_address_mode: mode: " << (int)(buffer[ 0 ]) <<std::endl;
    if( buffer[ 0 ] != 0 ) {
...
fob::load_flock_status( void )
{
	...
	int i = 0;
    for( i= 0; i < FOB_MAX_BIRDS; ++i ) {
        //prints out all status bits for each connected bird (debugging)
        std::cout<< "flock status: " << i << ": ";
        for( int j = 7; j >= 0; --j ) {
            std::cout<< ((buffer[ i ] >> j) & 0x1);
        }
        std::cout<< std::endl;
    ...
    for( unsigned int i = 0; i < m_birds.size( ); ++i ) {
		...
		std::cout << "flock status: " << i << ": ";
		for (int j = 7; j >= 0; --j) {
			std::cout << ((buffer[0] >> j) & 0x1);
		}
		for (int j = 7; j >= 0; --j) {
			std::cout << ((buffer[1] >> j) & 0x1);
		}
		std::cout << std::endl;
...
fob::check_error( void )
{
    ...
    if( (code == 31) || (code == 32) || (code == 6) ) {
        ...
        std::cout<< "fob: ignoring error: " << code << ": "
...
fob::fob( void ): m_error( true ), m_error_msg( "uninitialized" ),
    ...
    m_group( false ), m_save( 0x00 ), m_sleep( 500 ), 
...
fob::fob( const std::string& device_name, fob::hemisphere hemi,
    ...
    m_open( false ), m_device( -1 ), m_master( 1 ), m_fly( false ),
    m_group( false ), m_save( 0x00 ), m_sleep( sleep_ms  ),
    m_min_error_level( fob::FATAL )
{
    //create streaming thread
    if( pthread_create( &m_flock_thread, NULL, 
    ...
    fob::hemisphere hemi, fob::port_speed speed, unsigned long sleep_ms )
{
	...
	char portmode[] = { '8', 'N', '1', 0 };
	if (RS232_OpenComport(m_device, 115200, portmode))
	{
   ...
   /* settings.c_cflag = CS8 | CLOCAL | CREAD;
    ...
    if( !check_address_mode( ) ) {
        ...
        std::cout<< "fob::open: warning: addressing mode may be invalid" 
            << std::endl;
    }
...
{    
	...
	while (byte_read< 32 && read_error < 3)
	{
		byte_read += RS232_PollComport(m_device, &buffer[byte_read], 256 - byte_read);
    ...
    while( (phases_found < 2) && (read_error < 5) ) {
		...
		//if (RS232_PollComport(m_device, &buffer[i], 1) == 1) {
    ...
    if( (address < 0) || (address >= static_cast<int>( m_birds.size( ))) ) {
        ...
        //std::cout<< "fob::update: warning: bad bird address: "
	...
	m_birds[address]->update(&buffer[i], byte_read-i);
