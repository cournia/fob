...
public:
    ...
    class bird {
     private:
        ...
        glm::vec3 m_position; //!< Position of the bird.
        glm::vec3 m_fob_angles; //!< Orientation of the bird (in fob coords).
        glm::vec3 m_angles; //!< Orientation of the bird.
        glm::quat m_quaternion; //!< Orientation of the bird.
        glm::mat4 m_matrix; //!< Orientation/Position of the bird.
        glm::quat m_rotation; //!< Rotation to apply to the bird's orientation.
     ...
     public:
        ...
        inline void get_position( glm::vec3& output ) const {
            lock_data( );
            if( m_ori_dirty ) {
                output = m_position;
            }
        ...
        //! Gets the bird's position.
        inline void get_position( float output[ 3 ] ) const {
            lock_data( );
			std::memcpy(output, static_cast<const float*>(glm::value_ptr(m_position)), sizeof(float)* 3);
            unlock_data( );
        }
         ...
         */
        inline void get_angles( glm::vec3& output ) {
            lock_data( );
            if( m_ori_dirty ) {
                update_orientation( );
            }
		...
		* Rotations are relative to the fixed global coordinate frame.
		*/
		inline void get_rawangles(glm::vec3& output) {
			lock_data();
			if (m_ori_dirty) {
				update_orientation();
			}
         ...
         * Rotations are relative to the fixed global coordinate frame.
         */
        inline void get_angles( float output[ 3 ] ) {
            lock_data( );
            if( m_ori_dirty ) {
                update_orientation( );
            }
			std::memcpy(output, static_cast<const float*>(glm::value_ptr(m_angles)), sizeof(float)* 3);
            unlock_data( );
        }

        //! Gets the bird's orientation.
        inline void get_quaternion( glm::quat& output ) {
            lock_data( );
            if( m_ori_dirty ) {
                update_orientation( );
            }
         ...
         */
        inline void get_quaternion( float output[ 4 ] ) {
            lock_data( );
            if( m_ori_dirty ) {
                update_orientation( );
            }
			//std::memcpy(output, static_cast<const float*>(glm::value_ptr(m_quaternion.)), sizeof(float)* 3); 
			output[0] = m_quaternion.x;
			output[1] = m_quaternion.y;
			output[2] = m_quaternion.z;
         ...
         */
        inline void get_matrix(glm::mat4& output ) {
            lock_data( );
            if( m_ori_dirty ) {
                update_orientation( );
            }
         ...
         * The returned matrix is in row major format.
         */
        inline void get_matrix( float output[ 16 ] ) {
            lock_data( );
            if( m_ori_dirty ) {
                update_orientation( );
            }
			std::memcpy(output, static_cast<const float*>(glm::value_ptr(m_matrix)), sizeof(float)* 16);
            unlock_data( );
        }
 ...
 private:
    ...
    int m_device; //!< File handle to fob device (serial port).
     //termios m_save_tio; //!< Old saved for settings for fob serial port.
	...
	int write(int device, unsigned char* buffer, int numofbytes);
